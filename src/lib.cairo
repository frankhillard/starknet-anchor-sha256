...
mod tests {
   ...
   fn success_change_and_accept_admin() {
       ...
       start_prank(CheatTarget::One(contract_address), new_admin.try_into().unwrap());
   ...
   fn success_deploy() {
       ...
       let new_admin: felt252 = 0x02ce9c6303529f79e5bfbd09f9cf52421df68a962177977b8a8cc2074b84c097;
       start_prank(CheatTarget::One(factory_contract_address), admin.try_into().unwrap());
       factory_dispatcher.deploy(new_admin.try_into().unwrap());
       stop_prank(CheatTarget::One(factory_contract_address));
       ...
       let _anchor_address : felt252 = last_deployed_anchor.into();
   ...
   fn anchoring_authorize_success() {
       ...
       start_prank(CheatTarget::One(anchor_address), anchor_admin.try_into().unwrap());
       let _last_anchor = anchor_dispatcher.authorize(admin.try_into().unwrap());
       stop_prank(CheatTarget::One(anchor_address));
   }
   ...
   fn anchoring_unauthorize_success() {
       ...
       // Authorize another address
       start_prank(CheatTarget::One(anchor_address), anchor_admin.try_into().unwrap());
       let _last_anchor = anchor_dispatcher.unauthorize(anchor_admin.try_into().unwrap());
       stop_prank(CheatTarget::One(anchor_address));
   }
